...
const SuperadminPanel = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const token = localStorage.getItem('token');
      try {
        const response = await axios.get('https://sinfbackend1.onrender.com/api/dashboard', {
          headers: {
  ...
  const createUser = async () => {
    ...
    try {
      const response = await axios.post('https://sinfbackend1.onrender.com/api/dashboard',
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const deleteUser = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://sinfbackend1.onrender.com/api/users/${id}`,
